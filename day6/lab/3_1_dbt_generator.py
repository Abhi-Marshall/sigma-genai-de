...
def load_private_key_bytes() -> bytes:
    if not CRYPTO_AVAILABLE:
        raise RuntimeError(
            "cryptography is not installed. Install it with: python3 -m pip install cryptography"
        )
...
def get_connection():
    ...
    if not SNOWFLAKE_AVAILABLE:
        raise RuntimeError(
            ...
            "Install it with: python3 -m pip install snowflake-connector-python"
        )
...
def ask_cortex(question: str) -> dict:
    ...
    if not SNOWFLAKE_AVAILABLE or not CRYPTO_AVAILABLE:
        missing = []
        if not SNOWFLAKE_AVAILABLE:
        ...
        return {
            ...
            "rows": [],
            "elapsed_seconds": 0.0,
            "error": f"Missing dependencies: {', '.join(missing)}. Install them with: python3 -m pip install {' '.join(missing)}"
        }
